<%@ WebHandler Language="C#" Class="Handler_vista" %>
...
public class Handler_vista : IHttpHandler, IReadOnlySessionState {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (Directory.Exists(HttpContext.Current.Server.MapPath("~/Uploads/"+idproducto+"/")))
        {
            ...
            if (files.Length > 0)
            {
                sbArchivos.Append("<script>$('#file-5').fileinput('destroy'); $('#file-5').fileinput({ theme: 'fa5', language: 'es', showClose: false, showBrowse:false, showCaption: false, dropZoneTitle: false, showUpload: false, showRemove: false, initialPreviewShowDelete: false, maxFileSize: 15360, maxFileCount: 5, overwriteInitial: false, initialPreviewAsData: true, initialPreview: [");
                ...
                sbArchivos.Append("], initialPreviewDownloadUrl: \"Uploads/"+idproducto+"/{filename}\" })</script>");
            }
            else
            {
                sbArchivos.Append("");
